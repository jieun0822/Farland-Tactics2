...
{
	SOUND->addSound("전투", "sound/bgm09_전투.mp3", true, false);

	_player = new battleMap_player;
	_player->init();

	_monster = new battleMap_monster;
	_monster->init();
	...
	IMAGE->addImage("forestMap", "images/background_forest_1.bmp", WINSIZEX, WINSIZEY+5, true, RGB(255, 255, 255));
...
{
	//지은
	if (quest1_complete) townNight_active = true;
	...
	{
		if (PtInRect(&_monster->getKidRC(), m_ptMouse))
		{
		...
		if (IntersectRect(&temp, &(*_viMonster)->rc, &_player->getRect()))
		{
			...
			if ((_player->getCurrentState() == PLAYER_STATE::ATTACK1 || _player->getCurrentState() == PLAYER_STATE::ATTACK2))
			{
				...
				if ((*_viMonster)->hp <= 0)
				{
					...
					if (!isFirst) {			//활-길드방문이 처음이 아닐때만 애기 드랍
	...
	if (playerCollide)_player->setCollide(true);
...
void battleMap::render()
{
	...
	IMAGE->findImage("forestMap")->render(getMemDC(), 0, 0);
	...
	wsprintf(str, "guild isQuest : %d", _guild->getIsFirst());
	TextOut(getMemDC(), 10, 10, str, lstrlen(str));

}//render end.

void battleMap::changeKey()
{
	RECT temp;
...
void battleMap::lightGrayDraw(RECT rc)
{
...
void battleMap::shadowDraw(RECT rc)
{
...
void battleMap::lightDraw(RECT rc)
{
