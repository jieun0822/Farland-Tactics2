...
class battleMap_player : public gameNode
{
private:
	int x;
	int y;
...
public:
	...
	void setPlayerX(float x) { _img->setX(x); }
	int getPlayerX() { return _img->getX(); }
	void setPlayerY(float y) { _img->setY(y); }
	int getPlayerY() { return _img->getY(); }
