...
HRESULT progressBar::init(image* frontImage, image* backImage, float x, float y, int width, int height)
{
	_x = x;
	_y = y;

	_rcProgress = RectMake(x, y, width, height);
	_progressBarFront = frontImage;
	_progressBarBack = backImage;
...
{
	_progressBarBack->render(getMemDC(), x, y, 0, 0, _progressBarBack->getWidth(), _progressBarBack->getHeight());
	_progressBarFront->render(getMemDC(), x, y, 0, 0, _width, _progressBarBack->getHeight());
}

void progressBar::loadingRender()
{
	_progressBarFront->render(getMemDC(), _rcProgress.left, _y, 0, 0, _width, _progressBarBack->getHeight());
}
